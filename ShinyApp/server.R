...
bg <- readRDS("bigram3.RData"); tg <- readRDS("trigram3.RData"); qd <- readRDS("quadgram3.RData")
...
functionBigram <- function(the_word) {
  ...
  if (identical(character(0),as.character(head(bg[bg$w1 == the_word[1], 2], 1)))) {
    # testing print(bg$w1)
    message<<-"If no word found the most common word with highest frequency 'the' in English will be returned" 
    as.character(head("the",1))
  }
  else {
    message <<- "TRYING TO PREDICT THE WORD USING BIGRAM FREQUENCY MATRIX"
...
functionTrigram <- function(the_word) {
  ...
  if (identical(character(0),as.character(head(tg[tg$w1 == the_word[1]
    ...
    message<<- "TRYING TO PREDICT THE WORD USING TRIGRAM FREQUENCY MATRIX "
